...
int point1 = data.get(0).getSaleprice()/100;


%>

<div class="detail_title_area">
	<h3 class="name"><%=data.get(0).getPnm()%></h3>
	<p class="summary">
		<%=data.get(0).getPsub()%>
		<span class="detail_icons">
		</span>
	</p>
	<p class="seq_num Hide">상품번호 : <%=data.get(0).getIdx()%></p>
</div>
		...
		<div id="goods_thumbs">
			<div class="slides_container hide" style="position:relative;">
					<div class="viewImgWrap">
							<img src="<%=data.get(0).getFile1()%>" title="" alt="상품이미지"/>
					</div>
			</div>
		    ...
		    <ul class="goods_spec_sections">
				...
				<li class="deatil_price_area">
						<div class="deatil_sale_rate">
							<p class="inner">
								<span class="num"><%=data.get(0).getSale()%></span>%
							</p>
						</div>
						<p class="org_price">
							<span class="dst_th_size"><s><span class="num"><fmt:formatNumber value='<%=data.get(0).getPrice()%>' pattern="#,###"/></span></s>&#x20a9;</span>
						</p>
						<p class="sale_price">
								<input type="hidden" id="saleprice" name="saleprice" value='<%=data.get(0).getSaleprice()%>'>
								<span class="num"><fmt:formatNumber value='<%=data.get(0).getSaleprice()%>' pattern="#,###"/></span><span class='price_won'>&#x20a9;</span>
						</p>
				...
				<li class="goods_spec_number hide">
					<ul class="detail_spec_table">
						...
						<li class="td"><%=data.get(0).getIdx()%></li>
				...
				<li class="goods_spec_saving_benefit">
					<ul class="detail_spec_table">
						...
						<li>
							<span designElement="text" textIndex="45" >구매 시 1% 적립 </span>
							<input type="hidden" name="point" value='<%=point1%>'>
							<fmt:formatNumber value='<%=point1%>' pattern="#,###"/>&#x20a9;
				...
				<ul class="goods_buttons_section">
					<li>
						<ul class="basic_btn_area">
							...
							<li><button type="button" name ="buy_btn" id="buy_btn" class="btn_resp size_c color2" onclick="buy()"><span designElement="text" textIndex="99" >구매하기</span></button></li>
						</ul>
					</li>
	...
	<div class="goods_option_area">
		...
		<tr class="optionTr">
			<td colspan="2">
				...
				<select name=ea onchange="price(this.value)"> <!-- javascript price로 수량 넘기면서 해당 function 바로 실행시키기 -->
		...
		<!-- 선택된 옵션 노출 영역 끝 -->
				...
				<div class="goods_price_area">
					...
					<tr>
						...
						<td class="total_goods_price">
							...
							<input type="hidden" name="totalprice" id="totalprice" value="<%=data.get(0).getSaleprice()%>"/>
							<span id="total_goods_price"> <fmt:formatNumber value='<%=data.get(0).getSaleprice()%>' pattern="#,###"/>&#x20a9;</span>
						</td>
					</tr>
					</table>
				</div>
				...
				<div class="goods_buttons_area">
					<ul class="goods_buttons_section">
						<li>
							<ul class="basic_btn_area">
								<li><button type="button" class="btn_resp size_extra2 NpayNo" onclick="basket(<%=data.get(0).getIdx()%>)"><span designElement="text" >장바구니</span></button></li>
								<li><button type="button" name ="buy_btn" id="buy_btn2" class="btn_resp size_extra2 color2 NpayNo" onclick="buy()"><span designElement="text" >구매하기</span></button></li>
							</ul>
						</li>
					</ul>
				</div>
